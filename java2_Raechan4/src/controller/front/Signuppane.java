		...
		if( !( address.contains("시") && address.contains("구") && address.contains("동") ) ) {
    		...
    		if( result ) {
    			// 1. 메시지창 출력 [ Alert : 메시지(알람) 클래스 ]  
	    			...
	    			alert.setHeaderText("레트로 아케이드 커뮤니티에 어서오세요~!!!");
	    			alert.setContentText("회원가입 성공입니다");
	    			alert.showAndWait(); // 메시지 실행;
    			// 2. 화면 전환 [ 로그인페이지 전환 ]
	    			Front.instance.loadpage("/view/front/frontpane.fxml");
    		}else {
    			lblconfirm.setText(" [ 실패 ] DB오류[ 관리자에게 문의]해야합니다 ");
    			return; // 가입 못하게 [ 메소드 종료 ]
    		}
