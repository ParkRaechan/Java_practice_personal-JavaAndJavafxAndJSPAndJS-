		...
		while(true) { // 프로그램 실행 무한루프 [ 종료조건:x ]
			...
			if( ch == 1 ) {  // 회원가입 
				...
				if( logincheck == false ) { // 로그인성공시에는 아래 코드 실행불가 
					System.err.println(" 알림)) 동일한 회원정보가 없습니다.[로그이시패]");
				}
				
			} // 로그인 END 
			else if( ch==3 ) {// 아이디찾기
				// 1.이름과 연락처 입력받는다.
				System.out.println("---------아이디찾기페이지----------");
				System.out.println(" 이름 : ");
				String name = scanner.next();
				System.out.println(" 연락처 : ");
				String phone = scanner.next();
				// 2.배열내 동일한 이름과 연락처 있는지 확인

				boolean findCheck = false;
				for(Member temp : memberlist ) {
					if(temp!=null&&temp.name.equals(name)&&temp.phone.equals(phone)) {
						// 3.동일한 객체가 있으먄 해당 아이디출력
				...
				// 4.없으면 없다고 출력
				if(findCheck==false) {
					System.out.println("동일한 회원정보가 없습니다.[찾기실패]");
				}
			}
			else if( ch==4 ) { // 비밀번호찾기 
				System.out.println("---------비밀번호찾기페이지----------");
				...
				boolean findCheck = false;
				for(Member temp : memberlist ) {
					if(temp!=null&&temp.id.equals(name)&&temp.phone.equals(phone)) {
